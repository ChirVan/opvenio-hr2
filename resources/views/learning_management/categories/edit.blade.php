<x-app-layout>
    ...
    @endsection
                                       value="{{ old('category_slug', $category->category_slug) }}" required>    @section('sidebar')
    ...
    <div class="py-3">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg p-6">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="ml-1 text-gray-900 font-medium">Edit Assessment Category</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                ...
                <!-- Form -->
                <form method="POST" action="{{ route('learning.assessment.categories.update', $category) }}" class="space-y-8" id="assessmentCategoryForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="category_icon" name="category_icon" 
                                    ...
                                    <option value="bx-code-alt" {{ old('category_icon', $category->category_icon) == 'bx-code-alt' ? 'selected' : '' }}>💻 Code (Technical)</option>
                                    <option value="bx-user-voice" {{ old('category_icon', $category->category_icon) == 'bx-user-voice' ? 'selected' : '' }}>🗣️ Communication (Soft Skills)</option>
                                    <option value="bx-briefcase" {{ old('category_icon', $category->category_icon) == 'bx-briefcase' ? 'selected' : '' }}>💼 Briefcase (Business)</option>
                                    <option value="bx-medal" {{ old('category_icon', $category->category_icon) == 'bx-medal' ? 'selected' : '' }}>🏅 Medal (Certifications)</option>
                                    <option value="bx-brain" {{ old('category_icon', $category->category_icon) == 'bx-brain' ? 'selected' : '' }}>🧠 Brain (Cognitive)</option>
                                    <option value="bx-heart" {{ old('category_icon', $category->category_icon) == 'bx-heart' ? 'selected' : '' }}>❤️ Heart (Personal)</option>
                                </select>
                                @error('category_icon')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="color_theme" name="color_theme" 
                                    ...
                                    <option value="blue" {{ old('color_theme', $category->color_theme) == 'blue' ? 'selected' : '' }}>🔵 Blue (Professional)</option>
                                    <option value="green" {{ old('color_theme', $category->color_theme) == 'green' ? 'selected' : '' }}>🟢 Green (Growth)</option>
                                    <option value="red" {{ old('color_theme', $category->color_theme) == 'red' ? 'selected' : '' }}>🔴 Red (Important)</option>
                                    <option value="purple" {{ old('color_theme', $category->color_theme) == 'purple' ? 'selected' : '' }}>🟣 Purple (Creative)</option>
                                    <option value="orange" {{ old('color_theme', $category->color_theme) == 'orange' ? 'selected' : '' }}>🟠 Orange (Energetic)</option>
                                    <option value="teal" {{ old('color_theme', $category->color_theme) == 'teal' ? 'selected' : '' }}>🔷 Teal (Modern)</option>
                                    <option value="indigo" {{ old('color_theme', $category->color_theme) == 'indigo' ? 'selected' : '' }}>🔵 Indigo (Premium)</option>
                                    <option value="pink" {{ old('color_theme', $category->color_theme) == 'pink' ? 'selected' : '' }}>🩷 Pink (Friendly)</option>
                                </select>
                                @error('color_theme')
                                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                                @enderror
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            form.addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    const response = await fetch('{{ route("learning.assessment.categories.update", $category) }}', {
                        method: 'POST',
                        body: formData,
                        headers: {
